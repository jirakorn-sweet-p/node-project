...
router.get('/request-status',async (req,res) => {
    ...
    try{
        ...
        const request_doc = (await request_info.find({'student_code':'633020572-2'})).at(0);
        const data = (await student_info.find({'student_code':'633020572-2'})).at(0);
        const company = (await company_info.find({'student_code':'633020572-2'})).at(0);
